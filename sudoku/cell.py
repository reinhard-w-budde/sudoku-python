class Cell:
    ...
    def setFinalVal(self, val, step, ruleId):
        ...
        if self.step >= 0:
            raise RuntimeError("Cell " + self.toString() + " got a final value for the second time - logical error of a rule")
        elif self.isValPossible(val):
            self.possibleVals = [val]
            self.step = step
            self.ruleId = ruleId
        else:
            raise RuntimeError(self.toString() + " should be set to " + str(val) + ", but that is impossible")
    ...
    def getTheFinalVal(self):
        ...
        if size != 1:
            raise RuntimeError(self.toString() + " has no final value, but is asked for that")
    ...
    def toString(self):
        return "{" + str(self.getX()) + "," + str(self.getY()) + ";[" + ','.join(str(e) for e in self.possibleVals) + "]}"
